                    ...
                    <div class="card-body">
                        @if ($isSeriesEdit)
                            <div class="alert alert-warning mb-4">
                                ...
                                <strong>Recurring Booking:</strong> This is part of a series
                                ({{ $booking->series->reference_number }}).
                                Changes will apply to the <strong>entire series</strong>
                                ({{ $booking->series->bookings()->where('status', 'confirmed')->count() }} bookings).
                            </div>
                        @endif
                        ...
                        <form action="{{ route('my-bookings.update', $booking) }}" method="POST">
                                        ...
                                        @for ($hour = 8; $hour < 18; $hour++)
                                            @foreach (['00', '30'] as $minute)
                                                @php
                                                    ...
                                                    $currentStart = Carbon\Carbon::parse($booking->start_time)->format(
                                                        'H:i',
                                                    );
                                                @endphp
                                                <option value="{{ $time }}"
                                    ...
                                    <select class="form-select @error('end_time') is-invalid @enderror" id="end_time"
                                        ...
                                        @for ($hour = 8; $hour <= 18; $hour++)
                                            @foreach (['00', '30'] as $minute)
                                                ...
                                                @php
                                                    $time = sprintf('%02d:%s', $hour, $minute);
                                                    $currentEnd = Carbon\Carbon::parse($booking->end_time)->format(
                                                        'H:i',
                                                    );
                                                @endphp
                                                <option value="{{ $time }}"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('purpose') is-invalid @enderror" id="purpose" name="purpose" rows="3"
                                    maxlength="500" required>{{ old('purpose', $booking->purpose) }}</textarea>
                                @error('purpose')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <ul class="list-unstyled mb-0">
                            ...
                            @if ($isSeriesEdit)
                                <li class="mb-2">
                                    ...
                                    All {{ $booking->series->bookings()->where('status', 'confirmed')->count() }}
                        ...
                        fetch('{{ route('ajax.bookings.check-availability') }}', {
                            ...
                            .then(data => {
                                ...
                                if (data.available) {
                                    ...
                                    message.innerHTML =
                                        '<i class="bx bx-check-circle me-1"></i> Time slot is available!';
                                    submitBtn.disabled = false;
                                } else {
                                    message.className = 'alert alert-danger';
                                    message.innerHTML =
                                        `<i class="bx bx-x-circle me-1"></i> ${data.conflict?.message || 'Time slot is not available'}`;
