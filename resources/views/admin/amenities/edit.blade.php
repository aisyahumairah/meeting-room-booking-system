    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="mb-1">Edit Amenity: {{ $amenity->name }}</h4>
                    ...
                    <form action="{{ route('admin.amenities.update', $amenity) }}" method="POST">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="name" name="name"
                                        ...
                                        value="{{ old('name', $amenity->name) }}" placeholder="e.g., Projector, Whiteboard"
                                        required>
                                    @error('name')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select id="icon" name="icon"
                                        ...
                                        @foreach ($availableIcons as $iconClass => $iconName)
                                            <option value="{{ $iconClass }}" @selected(old('icon', $amenity->icon) == $iconClass)>
                                                {{ $iconName }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="description" name="description" rows="3"
                                class="form-control @error('description') is-invalid @enderror" placeholder="Brief description of the amenity...">{{ old('description', $amenity->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="bx bx-save me-1"></i> Update Amenity
                            </button>
                            <a href="{{ route('admin.amenities.index') }}" class="btn btn-outline-secondary">
                                Cancel
                            </a>
