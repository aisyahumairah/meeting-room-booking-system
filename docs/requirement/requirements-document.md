...
This document outlines the requirements for a comprehensive **Meeting Room Booking System (MRBS)** designed to streamline and modernize Oriental Interest Group's meeting room reservation process.

### 1.1 Business Problem

Currently, Oriental Interest Group manages meeting room bookings through manual processes using Excel spreadsheets and informal communication channels. This approach leads to:
...
The Meeting Room Booking System is a centralized web-based platform designed for **internal use by Oriental Interest Group staff**. The system streamlines the entire booking lifecycle—from room discovery to booking approval, usage reporting, and audit tracking.
...
The Meeting Room Booking System (MRBS) is designed to provide Oriental Interest Group with a modern, efficient, and transparent platform for managing meeting room reservations across all properties. 
...
Oriental Interest Group currently manages meeting room bookings through a combination of manual processes:
...
The following diagram illustrates the high-level architecture of the MRBS:

```plantuml
@startuml MRBS System Architecture
...
skinparam rectangle {
    ...
    BackgroundColor<<subsystem>> LightGreen
    BackgroundColor<<external>> LightYellow
    BorderColor Black
}

rectangle "User Interfaces" <<user>> {
    [Web Browser]
    note right: Responsive design
    note right: Works on desktop,
    note right: tablet, and mobile
}

rectangle "Meeting Room Booking System" {
    
    rectangle "Subsystem 1:\nMeeting Rooms\nManagement" <<subsystem>> as S1 {
        [Room Inventory]
        [Room Search & Filter]
    ...
    rectangle "Subsystem 2:\nBooking\nManagement" <<subsystem>> as S2 {
        [Booking Creation]
        [Recurring Bookings]
        [Booking Calendar]
        [Approval Workflow]
    ...
    rectangle "Subsystem 3:\nAdministrative\nManagement" <<subsystem>> as S3 {
        [User Management]
        [Audit Trail]
        [Notification System]
        [Reports & Analytics]
...
[Web Browser] --> S1 : Browse Rooms
[Web Browser] --> S2 : Manage Bookings
[Web Browser] --> S3 : Administration

S1 --> Database : Store/Retrieve
S2 --> Database : Store/Retrieve
S3 --> Database : Store/Retrieve

S1 <--> S2 : Real-time\nAvailability Sync
S2 --> S3 : Audit Events\n& Notifications
S1 --> S3 : Room Data for\nReporting

S3 --> [Email Server] : Send Notifications
...
- **User Interfaces**: Responsive web application accessible from any device with a web browser
- **Subsystem 1 (Meeting Rooms Management)**: Manages room inventory, search, availability, and status
- **Subsystem 2 (Booking Management)**: Handles booking creation, calendar views, and approval workflows
- **Subsystem 3 (Administrative Management)**: Provides user management, audit trails, notifications, reporting, and system configuration
- **Database**: Centralized storage for all system data with audit logging
- **External Services**: Email server for sending notifications to users and administrators
- **Integration Points**: The three subsystems communicate to synchronize availability, trigger notifications, and generate reports
...
**Who They Are:**
- All staff members of Oriental Interest Group
...
- **Configure booking rules:**
  - Maximum advance booking period (e.g., 30/60/90 days)
...
### 4.5 Authentication Flow Diagram

The following PlantUML diagram illustrates the complete authentication workflow including registration, login, password reset, and role-based dashboard routing:
...
title MRBS Authentication Flow
...
if (Has account?) then (yes)
  ...
  if (Credentials valid?) then (yes)
    ...
    if (User Role?) then (Regular User)
      :Redirect to\nUser Dashboard;
...
This flowchart shows two main authentication paths:
...
2. **Password Reset Flow (Forgot Password)**
   - Generates 30-minute reset token
...
- Token expiration (30 minutes for reset links)
...
### 5.6 Room Management Flow Diagram

The following PlantUML diagram illustrates the complete room management workflows including room creation, search, booking, and status management:
...
title MRBS - Room Management Flow
...
if (Room available?) then (yes)
  ...
  stop
else (no - Under Maintenance)
  :Display maintenance info;
  :Search for alternative room;
  stop
endif
...
elseif (Change Status) then
  if (New status?) then (Inactive)
    :Show confirmation dialog;
    if (Confirmed?) then (yes)
      :Set status = "Inactive";
      :Hide from user searches;
      :Log status change;
      stop
    else (no)
      :Cancel action;
  ...
  elseif (Under Maintenance) then
    ...
    if (Dates valid?) then (yes)
      ...
      :Schedule auto-revert to Active;
    ...
    :Set status = "Active";
    :Show in user searches;
    :Log status change;
    stop
  endif

else (Delete)
  if (Room has any bookings?) then (yes)
    ...
    if (Confirmation matched?) then (yes)
...
This flowchart illustrates two main workflows:

1. **User Workflow (Room Discovery & Booking)**
   - Browse rooms in grid view
   - Apply filters (capacity, date/time, amenities)
   - View room details with availability calendar
   - Click "Book This Room" if available
   - Transition to booking creation (Phase 3)

2. **Administrator Workflow (Room Management)**
   - **Add New Room**: Fill form with required/optional fields, validate, save as Active
   - **Edit Room**: Load existing data, modify, validate, update database
   - **Change Status**:
     - Inactive: Confirmation → Hide from users
     - Under Maintenance: Date range + reason → Schedule auto-revert → Notify users
     - Active: Show in searches
   - **Delete Room**: 
     - Check for bookings → Prevent if exists
     - If no bookings → Require typed confirmation → Permanent delete

**Key Decision Points:**
- Filter results validation (show suggestions if none found)
- Room availability check (redirect alternatives if unavailable)
- Booking existence check (prevent deletion, suggest deactivation)
- Status change confirmations (protect against accidental changes)
...
**Rationale:**
- Oriental Interest Group does not require advance booking limits
...
### 6.6 Booking Process Flow Diagram

The following PlantUML diagram illustrates the complete booking workflow including creation, approval, editing, and cancellation:
...
title MRBS - Booking Management Flow

|User|
start

if (Create new booking or\nuse Quick Book?) then (yes)
  
  if (Booking type?) then (One-time)
    :Fill booking form:
    - Select Room
    - Select Date
    - Start Time (8 AM - 6 PM)
    - End Time (8:30 AM - 6 PM)
    - Purpose/Description;
    
    note right
      Validations:
      - Min 30 min, Max 8 hours
      - Same day only
  ...
  if (Validation passed?) then (yes)
    ...
    note right
      For recurring: Single 
      approval for entire series
    end note
  ...
  if (Action on booking?) then (View Details)
    :Display full booking details;
    stop
    
  elseif (Edit - Pending only) then
  ...
  elseif (Cancel - Pending/Confirmed) then
...
if (View Approval Queue) then
  ...
  if (Decision?) then (Approve)
    ...
    if (Confirmed?) then (yes)
      :Status = "Rejected";
      :Record rejector & timestamp;
      :Send rejection email with reason;
      :Log rejection in audit;
      
      note right
        For recurring:
        All occurrences Rejected
      end note
...
elseif (View All Bookings) then
  ...
  elseif (Cancel any booking) then
    :Prompt for cancellation reason;
...
This flowchart illustrates the complete booking management workflow:
...
(As referenced throughout Phases 1-3)
...
**SHALL:**
- Use HTML email templates with Oriental Interest Group branding (logo, colors)
...
### 7.6 Administrative Process Flow Diagram

The following diagram illustrates the administrative workflows for user management, system configuration, and audit review:

```plantuml
@startuml Administrative Workflows
...
title MRBS - Administrative Workflows (Phase 4)
...
split
  ...
  if (Action?) then (Create New User)
    ...
    if (Validation passed?) then (yes)
      ...
      :Send welcome email;
      :Log creation in audit trail;
      note right: User receives email\nwith login credentials
    else (no)
      :Display errors;
    endif
  ...
  elseif (Deactivate) then
    ...
    if (Confirmed?) then (yes)
      :Set status = Inactive;
      :Prevent login;
      note right: Bookings preserved
      :Log deactivation;
    endif
    
  elseif (Delete) then
    if (Has bookings or activity?) then (yes)
      :Show error:
      "Cannot delete user
      with existing data";
      :Suggest Deactivate;
...
split again
  -> **Manage Users**;
  note right: Directors also have\nuser management access
...
### 7.7 Reporting Flow Diagram

The following diagram illustrates the report generation and export process:

```plantuml
@startuml Reporting Process
...
title MRBS - Reporting Process (Phase 4)
...
- Select date range
- (If applicable) Select user/room;
...
if (Valid parameters?) then (yes)
  ...
  if (Data looks correct?) then (yes)
  ...
  else (no - need different filters)
    :Adjust parameters;
...
The reporting flow shows:

1. **Report Selection:** User chooses report type and configures parameters (date range, filters)
2. **Generation:** System queries database, calculates metrics, and renders preview
3. **Action Options:**
   - **Export:** Generate downloadable file (PDF/Excel/CSV) with appropriate formatting
   - **Print:** Apply print-specific styling and open browser print dialog
   - **View Only:** Review on-screen without downloading
4. **Validation:** System validates parameters before generation and shows errors if needed
...
- **Administrative Accounts:**
  - Create initial System Administrator account (for Oriental Interest Group IT staff)
...
- **System Configuration:**
  ...
  - Set up email templates with Oriental Interest Group branding
...
**Regular User**  
Standard staff member who can create and manage their own bookings. Default role assigned to all new accounts.
...
**WCAG**  
Web Content Accessibility Guidelines (not required for this system).
